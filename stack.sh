...
if is_service_enabled q-agt; then
    if [[ "$Q_PLUGIN" = "openvswitch" || "$Q_PLUGIN" = "rgos" ]]; then
        # Install deps
...
if is_service_enabled quantum; then
    ...
    elif [[ "$Q_PLUGIN" = "rgos" ]]; then
        Q_PLUGIN_CONF_PATH=etc/quantum/plugins/rgos
        Q_PLUGIN_CONF_FILENAME=rgos_quantum_plugin.ini
        Q_DB_NAME="rgos_quantum"
        Q_PLUGIN_CLASS="quantum.plugins.rgos.rgos_quantum_plugin.RgosQuantumPlugin"
...
if is_service_enabled q-agt; then
	...
	if [[ "$Q_PLUGIN" = "rgos" ]]; then
        	AGENT_BINARY="$QUANTUM_DIR/quantum/plugins/rgos/agent/rgos_quantum_agent_ovs.py"
	else
